<?php
  ...
  function process($checkExisting = false){
    ...
    $sql = "SELECT data FROM battlejetsGames WHERE id = $gidx";
    ...
    if(mysqli_num_rows($res)){
      ...
      if($checkExisting){
        ...
        $sql = "UPDATE battlejetsGames SET data = \"$data\" WHERE id = $gameID";
  ...
  if(isset($gameID) && $gameID){
    $gidx = $gameID;
    if($userID){
      $sql = "SELECT * FROM battlejetsSession WHERE id = $userID";
      $res = mysqli_query($link, $sql);
      if(mysqli_num_rows($res)){
        process(true);
      }
