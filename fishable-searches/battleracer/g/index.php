      ...
      async function Draw(){
        ...
        if(!t){
          ...
          PlayerInit = (idx, uid) => { // called initially & when a player dies
            ...
            Players[idx].car              = spawnCar(0,0,0, uid)
            if(!cars.filter(v=>+v.id==+uid).length){
              cars=[...cars, Players[idx].car]
            }else{
              cars.filter(v=>+v.id==+uid)[0] = Players[idx].car
            }
          }
          ...
          spawnBullet = car => {
            floatingCam = !(+car.id == +userID) ? true : !car.camMode
            sd = curCar.curGun
            ls = car.curGun > 1 ? 1 : 0
            for(let i=sd; i--;){
          ...
          drawCar = (car, idx, col1='#0f83', col2="") => {
            ...
            if(car.grounded){
              if(!ocg && car.vy>1){
                ...
                if(car.vy > 6){
                  if(car.curGun>0){
                    spawnFlashNotice('CRASHED! [GUNS LOST]', '#800')
                    car.curGun = 0
                  }
                  car.decon.map(v=>{
              ...
              Array(rw*cl).fill().map((v, i) => {
                if(!((i+(i/cl/5|0))%2) && !((i/cl|0)%3)){
                  ...
                  if(d1<rw){
                    ay /= 6
                    alpha = 1 / (1+d1**8/1e15)
                    col1 = ``//`hsla(${ay*3},99%,50%,${.2}`
                    col2= `hsla(${ay*3+180},99%,25%,${.5}`
                    if(alpha>.1) stroke(col1, col2, 25, false, alpha)
                  }
                }
              })
            }
        ...
        if(flashNotices.length){
          ...
          x.fillText(flashNotices[l][0],c.width/2, c.height/2 - fs)
          flashNotices[l][2]-=.05
        }
              ...
              Array(rw*cl).fill().map((v, i) => {
                if(!((i+(i/cl/5|0))%2) && !((i/cl|0)%3)){
                  ...
                  if(Math.hypot(X,Z)<ls*1.00){
                    ...
                    R3(0,0,-cars[0].yw)
                    s = sp/1.5*2
                    x.fillStyle = `hsla(${fl*3+180},99%,50%,.25)`
                    ...
                    case 'car':
                      if(typeof el[key] != 'undefined' && +el[key].id != +userID){
                        ...
                        if(cars.filter(v=>+v.id == +el[key].id).length){
                          ...
                          let uid = el[key].id
                          Object.entries(incomingCar).forEach(([key,val]) => {
                            /*isNum = true
                            switch(key){
                              case 'shooting': isNum = false; break
                              case 'poweredUp': isNum = false; break
                              case 'playerName': isNum = false; break
                            }
                            if(isNum) val = +val*/
                            matchingCar[key] = val
                          })
                        ...
                        if(users.length < cars.length){
                          Players = Players.filter(player => {
                            cull = true
                            users.map(user =>{
                              if(+user.id == +player.id) cull = false
                            })
                            return !cull
                          })
                    ...
                    case 'powerups':
                      if(typeof el[key] != 'undefined'){
                        el[key].map(powerup => {
                          ...
                          let newPowerup = true
                          powerups.map(pup => {
                            if(+pup[0] == +X && +pup[2] == +Z) newPowerup = false
                          })
                          if(newPowerup){
        ...
        fetch('sync.php',{
          ...
          if(data[0]){
            ...
            if(data[2]){ //needs reg
              ...
              if(!gameConnected){
                setInterval(()=>{sync()}, pollFreq = 1200)  //ms
                gameConnected = true
              }
              if(!launched){
