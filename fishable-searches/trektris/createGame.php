<?php
  ...
  for($i=2; $i--;){
    $table = '';
    switch($i){
      case 0: $table = 'trektrisSessions'; break;
      //case 1: $table = 'trektrisGames'; break;
    }
    if($table){
  ...
  do{
    $gidx = 1e9 + (rand()%1e8);
    $sql = "SELECT id FROM trektrisGames WHERE id = $gidx";
    $res = mysqli_query($link, $sql);
  ...
  if($ct<1e3){
    ...
    $sql = "INSERT INTO trektrisSessions (name, data, gameID) VALUES(\"$sanitizedName\", \"[]\", $gidx)";
    mysqli_query($link, $sql);
    $userID = mysqli_insert_id($link);
    $data = mysqli_real_escape_string($link, newUserJSON($userName, $userID));
    $sql = "INSERT INTO trektrisGames (data, id) VALUES(\"$data\", $gidx)";
    mysqli_query($link, $sql);
    $success = true;
