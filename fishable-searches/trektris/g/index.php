...
<html>
  <head>
    <title>TREKTRIS! multiplayer/online ARENA</title>
    <style>
      /* latin-ext */
      @font-face {
      ...
      */
      ...
      async function Draw(){
        ...
        if(!t){
          ...
          tboard = new Image()
          tboard.src = '/games_shared_assets/tboard.png'
          tboard_inactive = new Image()
          tboard_inactive.src = '/games_shared_assets/tboard_inactive.jpg'
          gridoverlay = new Image()
          gridoverlay.src = '/games_shared_assets/trektris_gridoverlay.png'
          border = new Image()
          border.src = '/games_shared_assets/trektris_border.png'
          ...
          spawnPlayer = (uid, max=0, speed=10, tetrises=0, respawns=0, newBoard=true) =>{
            let ret = {
          ...
          resetGame = (player, idx) => {
            player.deathGrow = 0
            let tplayer = spawnPlayer(0, player.maxRowsCompleted, player.speed, player.tetrises, player.respawns, false)
            if(!idx){
        ...
        if(gameInPlay){
          players.map((player, idx) => {
            ...
            drawPiece(player, idx)
            if(player.alive){
              if(!idx) doKeys(player, idx)
              if(!idx) drawDropShadow(player, idx)
              if(!paused && !idx){ //(idx>1 || !idx)){
        ...
        */
        ...
        X = c.width * .75
        Y = c.height-100
        renderButton('🔗', X, Y, '  copy shareable link to this game-in-progress', 'fullCopy()', 'rectangle', '#0ff4', '#2088', 64)
        ...
        fetch('sync.php',{
          ...
          if(data[0]){
            ...
            if(data[2]){ //needs reg
              ...
              if(!gameConnected){
                setInterval(()=>{sync()}, pollFreq = 700)  //ms
                gameConnected = true
              }
              if(!launched){
